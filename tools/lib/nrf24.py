...
class nrf24:
  ...
  def enter_promiscuous_mode(self, prefix=[], rate=RF_RATE_2M, addrlen=5):
    print(rate)
    self.send_usb_command(ENTER_PROMISCUOUS_MODE, [rate, addrlen, len(prefix)]+list(map(ord, prefix)))
  ...
  def enter_sniffer_mode(self, address, rate=RF_RATE_2M):
    print(rate)
    self.send_usb_command(ENTER_SNIFFER_MODE, [rate, len(address)]+list(map(ord, address)))
    self.dongle.read(0x81, 64, timeout=nrf24.usb_timeout)
